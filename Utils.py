...
def analyze_storm_exp(path_to_model, exp_class, hidden_channels, num_layers, scale, device):
    ...
    for i in range(X_test.size(0)):
        out, _ = model(X_test[i:i + 1], torch.flip(X_test[i:i + 1], dims=[1]), inference=True)

        curr_vid = np.zeros([1, T, C, H, W])
        for j in tqdm(range(T)):
            curr_vid[0, j] = 255 * normalize_input_01(out[0, j])
...
def analyze_storm_exp_overlap(path_to_model, exp_class, hidden_channels, num_layers, scale, device):
    ...
    for i in range(X_test.size(0)):
        ...
        for j in range(X_test.shape[1]):
            out.append(model(X_test[i:i + 1, j:j+exp_class.sum_factor],
                             torch.flip(X_test[i:i + 1, j:j+exp_class.sum_factor], dims=[1]), inference=True))
...
def analyze_storm_exp_one_directional(path_to_model, exp_class, hidden_channels, num_layers, scale, device):
    ...
    for i in range(X_test.size(0)):
        out, _ = model(X_test[i:i + 1], inference=True)
