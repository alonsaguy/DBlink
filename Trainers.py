...
class LSTM_overlap_Trainer:
    def __init__(self, model, loss_fn, optimizer, scheduler, batch_size, window_size, patience, device):
        self.model = model
        self.loss_fn = loss_fn
        self.optimizer = optimizer
        self.scheduler = scheduler
    ...
    def train_epoch(self, dl_train, **kw):
        ...
        for X_train, y_train in tqdm(dl_train):
            ...
            for j in range(X_train.shape[1]):
                out.append(self.model(X_train[:, j:j+self.window_size],
                                      torch.flip(X_train[:, j:j+self.window_size], dims=[1])))
        ...
        with torch.no_grad():
            for X_test, y_test in tqdm(dl_test):
                ...
                for j in range(X_test.shape[1]):
                    out.append(self.model(X_test[:, j:j + self.window_size],
                                          torch.flip(X_test[:, j:j + self.window_size], dims=[1])))
